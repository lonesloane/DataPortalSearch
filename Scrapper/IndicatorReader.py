...
def populate_doi_keywords_dict():
    for url in indicatorsList:
        ...
        for publication_doi in root.findall('response:data/infoSet/indicator/item[@type="relatedPublication"]/target', ns):
            if DEBUG: print "related Publication: " + publication_doi.text
            doi_keywords_dict[publication_doi.text] = doi_keywords
        for publication_doi in root.findall('response:data/infoSet/indicator/item[@type="sourcePublication"]/target', ns):
